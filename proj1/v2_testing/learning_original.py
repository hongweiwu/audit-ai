...
def init_program():
    ...
    global K; from keras import backend as K
...
def load_data_binary():
    f1 = open('data/pos-200.csv', 'r' ,encoding='utf-8')
    f2 = open('data/neg-200.csv', 'r' ,encoding='utf-8')
...
def load_data_multiclass():
    f1 = open('data/neg-200.csv', 'r', encoding='utf-8')
    r1 = csv.reader(f1, delimiter=',', quotechar='"')

    data = process_m(np.array(list(r1)))
    x_data = normalize(np.delete(data, [5], axis=1))
    y_data = data[:, 5]
    len_data = data.shape[0] // 1280 * 1280 #为何一定要得到 1280 的整数倍？//表示除法后取整数值
...
def main():
    ...
    if args.binary: #hongwei 估计是只训练 nn 如何判断 0、1
        print("passed in argument is binary");
        num_classes = 2
        x_data, y_data = load_data_binary()
    else: #hongwei 训练 nn 判断不好的情况下到底属于那种 violations/舞弊 
