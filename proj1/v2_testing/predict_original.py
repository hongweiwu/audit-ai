...
def load_data():
    r = csv.reader(open('data/predict.csv', 'r' ,encoding='utf-8'),
            delimiter=',', quotechar='"')
...
def predict(model_b, model_m, x_data, num_list):
    ...
    #import shared as s
    #s.print_data("pdata_m",model_m.predict(x_data[rows]))
    #exit()
...
def main():
    ...
    f = open('suspicious.txt', 'w', encoding='utf-8')
    sus, kind = predict(model_b, model_m, x_data, num_list)
    import shared as s
    s.print_data("sus",sus)
    s.print_data("kind",kind)
